...
class ApiVentas(api):
    ...
    def generar_ticket(self, periodo):
        url = (f"https://api-sire.sunat.gob.pe/v1/contribuyente/migeigv/libros/rvie/propuesta/web/propuesta/"
               f"{periodo}/exportapropuesta?codTipoArchivo=0")
    ...
    def download_archivo(self, nomArchivo, codTipo):
        ...
        try:
            ...
            url = (f"https://api-sire.sunat.gob.pe/v1/contribuyente/migeigv/libros/rvierce/"
                   ...
                   f"nomArchivoReporte={nomArchivo}&codTipoArchivoReporte={codTipo}&codLibro=140000&"
                   f"perTributario={super().get_periodo()}&codProceso=10&numTicket={super().get_ticket()}")

            msg_error = 'Descargar Archivo'

            response = super().send_request("get", url, headers, None)
    ...
    def esperar_estado_terminado(self):
        max_espera = 120  # Máximo de 10 minutos
        ...
        for i in range(iteraciones):
            ...
            for estado in estados:
                if estado["desEstadoProceso"] == "Terminado" and estado.get("archivoReporte"):
                    print("\nSUNAT completo el proceso. Iniciando descarga de archivo(s)...")
                    sys.stdout.flush()
    ...
    def Ejecucion_ApiVentas(self):
        ...
        print("\nESPERANDO QUE SUNAT PREPARE EL ARCHIVO PARA DESCARGAR...")
        sys.stdout.flush()
